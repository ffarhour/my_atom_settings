...
void asp::datapath()
{
    ...
    if(store_cmd){
        R_CMD = data_in;
        opcode = R_CMD.range(25,22);
        cmd_mem_sel = R_CMD.get_bit(17);
        m = R_CMD.range(8,0);
        n = R_CMD.range(17,9);
    }
...
void asp::dataout_clk_sync()
{
    while(true){
        wait();
        result_split_count_sig = result_split_count;
        if(res_ready){

            if(result_split_count_sig.read()==0){
                data_out = result.range(15,0).to_uint();
            }else if(result_split_count_sig.read()==1){
                data_out = result.range(31,16).to_uint();
                wait();
                data_out = result.range(15,0).to_uint();
            }else if(result_split_count_sig.read()==2){
                data_out = result.range(47,32).to_uint();
                wait();
                data_out = result.range(31,16).to_uint();
                wait();
                data_out = result.range(15,0).to_uint();
...
void asp::alu()
{
    if(reset_AB){ // Reset
        ...
        if(cmd_mem_sel.read() == 0){
            A[data_in.read().range(24,16).to_uint()] = data_in.read().range(15,0);
        }else{
            B[data_in.read().range(24,16).to_uint()] = data_in.read().range(15,0);
        }
        ...
        result = result_temp;
        if(result_temp<65536){
            result_split_count = 0;
        }else if(result_temp<4294967296ULL){
            result_split_count = 1;
        }else{
            result_split_count = 2;
        }
...
void asp::next_state_logic()
{
    switch(curr_state.read())
    {
        ...
        case s_mac2:
            if(counter_comp_zero && (result_split_count>0)){
                next_state = s_mac3;
            } else if(counter_comp_zero){
                next_state = s_read_cmd;
            } else{
                next_state = curr_state;
            }
